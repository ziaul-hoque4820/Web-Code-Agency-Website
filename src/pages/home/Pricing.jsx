...
function Pricing() {
    ...
    return (
        <div className='section-container'>
            <HeadingSection heading="Choose Your Plan" subheading="Pricing" description="Our pricing plans are flexible and disigned  to fit your needs. Choose a plan that works best for you" />

            {/* toogle for yearly/monthly pricing  */}
            <div className='text-center mb-10'>
                <label className='inline-flex items-center cursor-pointer'>
                    ...
                    <div className='relative'>
                        ...
                        <div className={`toggle-circle absolute top-1 left-1 w-6 h-6
                        rounded-full bg-blue-600 transition-transform duration-300 ${isYearly ? "transform translate-x-6" : ""}`}></div>
                    </div>
                </label>
            </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10'>
                {
                    pricingPlans.map((plan, index) => (
                        <div key={index} className='bg-white border rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl'>
                            <div className='p-5 text-center'>
                                <h3 className='text-2xl font-semibold text-gray-700 my-3'>{plan.name}</h3>
                                <p className='text-gray-500 mb-5'>Best for {plan.name.toLocaleLowerCase()}</p>
